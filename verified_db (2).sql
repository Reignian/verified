...
CREATE TABLE `account` (
  `id` int(11) NOT NULL,
  `account_type` enum('student','institution','admin','institution_staff') NOT NULL,
...
CREATE TABLE `activity_log` (
  ...
  `user_type` enum('institution','institution_staff') NOT NULL,
  `action` varchar(100) NOT NULL,
  `action_type` enum('credential_issued','credential_deleted','credential_viewed','student_added','student_deleted','student_imported','profile_updated','address_updated','staff_added','staff_deleted','program_added','program_deleted','login','logout','settings_viewed','other') NOT NULL DEFAULT 'other',
...
CREATE TABLE `contact_messages` (
  ...
  `message_type` enum('contact','signup_request') NOT NULL DEFAULT 'contact',
...
--
-- Table structure for table `credential_type_samples`
--

CREATE TABLE `credential_type_samples` (
  `id` int(11) NOT NULL,
...
CREATE TABLE `credential_validation_logs` (
  ...
  `validation_result` enum('valid','invalid','error') NOT NULL,
  `ai_confidence` decimal(5,2) DEFAULT NULL,
  `ai_reasoning` text DEFAULT NULL,
  `detected_type` varchar(100) DEFAULT NULL,
...
ALTER TABLE `activity_log`
  ...
  ADD KEY `idx_institution_date` (`institution_id`,`created_at`);
...
--
-- Indexes for table `credential_type_samples`
--
ALTER TABLE `credential_type_samples`
  ADD PRIMARY KEY (`id`),
  ADD KEY `credential_type_id` (`credential_type_id`),
...
--
-- AUTO_INCREMENT for table `credential_type_samples`
--
ALTER TABLE `credential_type_samples`
...
--
-- Constraints for table `credential_type_samples`
--
ALTER TABLE `credential_type_samples`
  ADD CONSTRAINT `credential_type_samples_ibfk_1` FOREIGN KEY (`credential_type_id`) REFERENCES `credential_types` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `credential_type_samples_ibfk_2` FOREIGN KEY (`uploaded_by`) REFERENCES `account` (`id`);
