...
export const fetchActivityLogs = async (institutionId, filter = 'all') => {
  try {
    const url = filter === 'all'
      ? `${API_URL}/api/institution/${institutionId}/activity-logs`
      : `${API_URL}/api/institution/${institutionId}/activity-logs?action=${filter}`;
...
export const logActivity = async (logData) => {
  try {
    const response = await axios.post(
      `${API_URL}/api/institution/activity-logs`,
      logData
    );
    return response.data;
  } catch (error) {
